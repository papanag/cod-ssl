...
def test_vcod_notebooks_are_thin_valid_colab_orchestrators():
    ...
    assert "sys.path.insert(0, project_src)" in sources[names[1]]
    assert "from mamba_ssm import Mamba" in sources[names[1]]
    assert "QUEUE_INDEX == -1" in sources[names[1]]
    assert "TRAINING_COMPLETE" in sources[names[1]]
    assert "EVALUATION_COMPLETE" in sources[names[1]]
    assert "vcod_tuning_selection.json" in sources[names[1]]
    assert "TUNING_STAGES = (250, 1000, 3000)" in sources[names[1]]
    assert "--output-dir" in sources[names[1]]
    assert "RUN_KIND == 'exploratory'" in sources[names[1]]
    assert "EXPLORATORY_STEPS = 75" in sources[names[1]]
    assert "EXPLORATORY_TRAIN_TARGETS = 512" in sources[names[1]]
    assert "EXPLORATORY_VAL_TARGETS = 256" in sources[names[1]]
    assert "EXPLORATORY_SUBSET_SEED = 42" in sources[names[1]]
